defmodule TxDashboard.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      # Start the Ecto repository
      TxDashboard.Repo,
      # Start the Telemetry supervisor
      TxDashboardWeb.Telemetry,
      # Start the PubSub system
      {Phoenix.PubSub, name: TxDashboard.PubSub},
      # Start the Endpoint (http/https)
      TxDashboardWeb.Endpoint,
      # Start a worker by calling: TxDashboard.Worker.start_link(arg)
      # {TxDashboard.Worker, arg}
      # Start rabbit consumer
      TxDashboard.Consumer
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: TxDashboard.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    TxDashboardWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
